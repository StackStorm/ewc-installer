...
BASE_PATH="https://raw.githubusercontent.com/extremenetworks/ewc-installer"
...
if [ ! -z ${SUITE} ] && ${PKG_TYPE}_is_installed bwc-enterprise bwc-ui; then
    echo 'Enterprise is already installed.'
    echo 'Proceeding with Automation Suites install ...'
    ...
    rc=${PIPESTATUS[0]}
    if [ ${rc} -ne 0 ]; then
      echo "Enterprise failed to install."
      echo "Please contact support@extremenetworks.com with installation logs if you have any questions."
      exit 2
    fi
fi
...
if [ ! -z ${SUITE} ]; then
  ...
  if [ $? -ne 0 ]; then
      ...
      echo "Running deployment script for Automation Suites ${VERSION}..."
      ...
      rc=${PIPESTATUS[0]}
      if [ ${rc} -ne 0 ]; then
        echo "Automation Suites failed to install."
        echo "Please contact support@extremenetworks.com with installation logs if you have any questions."
        exit 3
      fi
  fi
