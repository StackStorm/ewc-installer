...
BWC_SVC_NAME="network-topology"
BWC_INSTALL_DIR="${BROCADE_INSTALL_DIR}/${BWC_SVC_NAME}"
BWC_LOG_DIR="/var/log/brocade/bwc"
BWC_CONFIG_DIR="/etc/brocade/bwc"
BWC_CONFIG_FILE="${BWC_CONFIG_DIR}/network-topology-service.conf"
BWC_DB_SETUP_SCRIPT="${BWC_INSTALL_DIR}/bin/network_topology_db_setup.sh"
BWC_DB_PASSWORD=""
ST2_API_KEY=""
...
setup_args() {
  for i in "$@"
    ...
    if [[ -z "${ST2_API_KEY}" ]]; then
        echo "INFO: The --st2-api-key option is not provided. A new st2 API key will be created for network-topology."
...
echo "INFO: Configuring Network topology database..."
